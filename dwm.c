...
void destroynotify(XEvent *e) {
  ...
  if ((c = wintoclient(ev->window)))
    unmanage(c, 1);
  /* 	else if (usealtbar && (m = wintomon(ev->window)) && m->barwin ==
   * ev->window) */
...
void runautostart(void) {
  ...
  if (xdg_config && *xdg_config) {
    autostart_path = "%s/dwm/scripts/autostart.sh";
  } else if (home) {
    autostart_path = "%s/.config/dwm/scripts/autostart.sh";
  } else {
    return;
  }

  snprintf(full_path, sizeof(full_path), autostart_path,
           (xdg_config ? xdg_config : home));

  if (access(full_path, X_OK) == 0) {
    char cmd[512 + 2];
    snprintf(cmd, sizeof(cmd), "%s &", full_path);
    system(cmd);
  }
}
